...
$themesDir   = "$mainfolder\Tools\Launcher\Themes"
...
function Read-Directories {
    $defaults = @{
        ...
        Auth     = "$mainfolder\Servers\realmd.exe"
        World    = "$mainfolder\Servers\mangosd.exe"
        ...
        AuthConf     = "$mainfolder\Servers\realmd.conf"
        WorldConf    = "$mainfolder\Servers\mangosd.conf"
    }
...
function Read-Theme {
    # Fallback defaults — used only if theme preset file is missing
    $d = @{
        BgWindow      = '#0C1610'
        BgSidebar     = '#120808'
        BgCard        = '#1E1212'
        ...
        ConsoleColor  = '0E'
        ActiveTheme   = 'Exp1'
        AppIcon       = 'Launcher.ico'
    }
    # Step 1: read Theme.txt — contains ActiveTheme selector (and optional overrides)
    ...
    # Step 2: load color values from the active preset file in Tools\Launcher\Themes\
    $preset = "$themesDir\$($d.ActiveTheme).txt"
    if (Test-Path $preset) {
...
function Stop-AuthServer  { Stop-Process -Name "realmd"  -Force -ErrorAction SilentlyContinue }

function Start-WorldServer {
    if (Test-Proc "mangosd") { return }
...
function Send-WorldCommand([string]$cmd) {
    $proc = Get-Process -Name "mangosd" -ErrorAction SilentlyContinue | Select-Object -First 1
    if (-not $proc) { return }
...
function Stop-WorldServer([scriptblock]$OnComplete = $null) {
    ...
    $Script:shutdownTimer.Add_Tick({
        ...
        if ($Script:shutdownPhase -eq 1 -and $Script:shutdownTicks -ge 3) {
            ...
            if (-not (Test-Proc "mangosd") -or $Script:shutdownTicks -ge 30) {
                $Script:shutdownTimer.Stop()
                Stop-Process -Name "mangosd" -Force -ErrorAction SilentlyContinue
...
function Invoke-ExportSave([int]$slot, [string]$slotName) {
    ...
    cmd /c "`"$dump`" --defaults-extra-file=`"$conf`" --default-character-set=utf8mb4 --routines --events --databases classicrealmd --add-drop-database > `"$d\auth.sql`"" 2>&1 | Out-Null
    cmd /c "`"$dump`" --defaults-extra-file=`"$conf`" --default-character-set=utf8mb4 --routines --events --databases classiccharacters --add-drop-database > `"$d\characters.sql`"" 2>&1 | Out-Null
    return $true
}
            ...
            <Grid>
              ...
              <StackPanel Grid.Column="0" Orientation="Horizontal" VerticalAlignment="Center">
                ...
                <StackPanel>
                  ...
                  <TextBlock Name="SrvLblAuth" Text="realmd.exe - Stopped" Foreground="#9E5555" FontSize="11"/>
                </StackPanel>
              </StackPanel>
              <StackPanel Grid.Column="1" Orientation="Horizontal">
            ...
            <Grid>
              ...
              <StackPanel Grid.Column="0" Orientation="Horizontal" VerticalAlignment="Center">
                ...
                <StackPanel>
                  ...
                  <TextBlock Name="SrvLblWorld" Text="mangosd.exe - Stopped" Foreground="#9E5555" FontSize="11"/>
                </StackPanel>
              </StackPanel>
              <StackPanel Grid.Column="1" Orientation="Horizontal">
...
<Button Name="BtnHeidSQL"    Content="Database Tool" Style="{StaticResource BtnGray}" Margin="0,0,8,6"/>
          ...
          <Border Style="{StaticResource Card}">
            <StackPanel>
              <TextBlock Text="Launcher Theme" Foreground="#FFF2F2" FontSize="13" FontWeight="SemiBold" Margin="0,0,0,14"/>
              <StackPanel Orientation="Horizontal">

                <Border Name="ThemeCard1" Background="#2A1515" CornerRadius="8"
                        BorderThickness="2" BorderBrush="#4E5058" Padding="14,12" Margin="0,0,12,0" MinWidth="172">
                  <StackPanel>
                    <Rectangle Height="5" RadiusX="2" RadiusY="2" Fill="#FFD100" Margin="0,0,0,10"/>
                    <TextBlock Text="Classic" Foreground="#FFF2F2" FontSize="13" FontWeight="SemiBold" Margin="0,0,0,10" HorizontalAlignment="Center"/>
                    <Button Name="BtnTheme1" Content="Apply Theme"
                            Style="{StaticResource BtnGreen}" HorizontalAlignment="Stretch"/>
                    <TextBlock Name="ThemeActive1" Text="* Active" Foreground="#FFD100"
                               FontSize="10" FontWeight="SemiBold" Margin="0,6,0,0"
                               HorizontalAlignment="Center" Visibility="Collapsed"/>
                  </StackPanel>
                </Border>

                <Border Name="ThemeCard2" Background="#2A1515" CornerRadius="8"
                        BorderThickness="2" BorderBrush="#4E5058" Padding="14,12" Margin="0,0,12,0" MinWidth="172">
                  <StackPanel>
                    <Rectangle Height="5" RadiusX="2" RadiusY="2" Fill="#56E05F" Margin="0,0,0,10"/>
                    <TextBlock Text="The Burning Crusade" Foreground="#FFF2F2" FontSize="13"
                               FontWeight="SemiBold" TextWrapping="Wrap" Margin="0,0,0,10"
                               HorizontalAlignment="Center" TextAlignment="Center"/>
                    <Button Name="BtnTheme2" Content="Apply Theme"
                            Style="{StaticResource BtnGreen}" HorizontalAlignment="Stretch"/>
                    <TextBlock Name="ThemeActive2" Text="* Active" Foreground="#56E05F"
                               FontSize="10" FontWeight="SemiBold" Margin="0,6,0,0"
                               HorizontalAlignment="Center" Visibility="Collapsed"/>
                  </StackPanel>
                </Border>

                <Border Name="ThemeCard3" Background="#2A1515" CornerRadius="8"
                        BorderThickness="2" BorderBrush="#4E5058" Padding="14,12" MinWidth="172">
                  <StackPanel>
                    <Rectangle Height="5" RadiusX="2" RadiusY="2" Fill="#58B4F0" Margin="0,0,0,10"/>
                    <TextBlock Text="Wrath of the Lich King" Foreground="#FFF2F2" FontSize="13"
                               FontWeight="SemiBold" TextWrapping="Wrap" Margin="0,0,0,10"
                               HorizontalAlignment="Center" TextAlignment="Center"/>
                    <Button Name="BtnTheme3" Content="Apply Theme"
                            Style="{StaticResource BtnGreen}" HorizontalAlignment="Stretch"/>
                    <TextBlock Name="ThemeActive3" Text="* Active" Foreground="#58B4F0"
                               FontSize="10" FontWeight="SemiBold" Margin="0,6,0,0"
                               HorizontalAlignment="Center" Visibility="Collapsed"/>
                  </StackPanel>
                </Border>
...
# Window icon — resolved from the active theme's AppIcon key
$iconPath = "$iconsDir\$($Script:theme.AppIcon)"
...
$_gh   = $Script:theme.AccentGreen.TrimStart('#')
$GREEN = [Windows.Media.SolidColorBrush]([Windows.Media.Color]::FromRgb(
    [Convert]::ToByte($_gh.Substring(0,2),16),
    [Convert]::ToByte($_gh.Substring(2,2),16),
    [Convert]::ToByte($_gh.Substring(4,2),16)))
...
$Script:hwndAuth  = [IntPtr]::Zero   # HWND of embedded realmd console
$Script:hwndWorld = [IntPtr]::Zero   # HWND of embedded mangosd console
...
function Sync-Consoles {
    ...
    foreach ($item in @(
        @{ ProcName = "realmd";   Placeholder = $ConsoleAuth  },
        @{ ProcName = "mangosd";  Placeholder = $ConsoleWorld }
    )) {
        $isBnet = $item.ProcName -eq "realmd"
...
function Refresh-Status {
    $db    = Test-Proc "mysqld"
    $bnet  = Test-Proc "realmd"
    $world = Test-Proc "mangosd"
    $web   = Test-Proc "httpd"
    ...
    $SrvLblAuth.Text  = "realmd.exe - "  + $SrvLblAuth.Text
    $SrvLblWorld.Text = "mangosd.exe - " + $SrvLblWorld.Text
...
function New-AccountSRP6([string]$username, [string]$password) {
    # CMaNGOS uses WoW SRP6: v = g^x mod N
    # where x = SHA1(s_le || SHA1(UPPER(username) + ":" + UPPER(password)))
    # s and v are stored as uppercase big-endian hex text (BN_hex2bn convention).
    # AsByteArray(32, littleEndian=true) reverses them before sending to the client,
    # so we store them reversed (BE) so the client receives the correct LE bytes.
    ...
    # 1. Random 32-byte salt (LE — what the client will receive after server reversal)
    $saltLE = [byte[]]::new(32)
    $rng.GetBytes($saltLE)
    ...
    # 3. x = SHA1(saltLE || H(I))
    $x_bytes = $sha1.ComputeHash([byte[]]($saltLE + $h_I))
    $x = [System.Numerics.BigInteger]::new([byte[]]($x_bytes + [byte]0x00))

    # 4. v = g^x mod N  (LE bytes, padded to 32)
    $v_raw    = [System.Numerics.BigInteger]::ModPow($g, $x, $N).ToByteArray()
    $verifierLE = [byte[]]::new(32)
    [Array]::Copy($v_raw, $verifierLE, [Math]::Min($v_raw.Length, 32))

    # 5. Reverse to big-endian for storage (BN_hex2bn reads MSB first)
    $saltBE = $saltLE.Clone(); [Array]::Reverse($saltBE)
    $verBE  = $verifierLE.Clone(); [Array]::Reverse($verBE)

    return @{
        SHex = ($saltBE | ForEach-Object { $_.ToString('X2') }) -join ''
        VHex = ($verBE  | ForEach-Object { $_.ToString('X2') }) -join ''
    }
}

function Get-Accounts {
    $rows = Invoke-AuthQuery "SELECT id, username, email, gmlevel FROM classicrealmd.account ORDER BY username;"
    if (-not $rows) { return @() }
    $rows | ForEach-Object {
        $c = $_ -split "`t"
        if ($c.Count -ge 4) {
...
'@
    ...
    $d.FindName("BtnCreate").Add_Click({
        ...
        $existsUser = Invoke-AuthQuery "SELECT COUNT(*) FROM classicrealmd.account WHERE username='$uEsc';"
        if ($existsUser -and [int]($existsUser.Trim()) -gt 0) { $errMsg.Text = "Username already exists."; $errMsg.Visibility = "Visible"; return }

        $existsEmail = Invoke-AuthQuery "SELECT COUNT(*) FROM classicrealmd.account WHERE email='$eEsc';"
        ...
        # Create account with SRP6 s+v (CMaNGOS: big-endian hex text, expansion=0 for Classic)
        $srp = New-AccountSRP6 $u $p
        Invoke-AuthQuery "INSERT INTO classicrealmd.account (username,s,v,email,gmlevel,joindate,expansion) VALUES ('$uEsc','$($srp.SHex)','$($srp.VHex)','$eEsc',$lvl,NOW(),0);" | Out-Null

        # Register account in realmcharacters for every realm
        Invoke-AuthQuery "INSERT INTO classicrealmd.realmcharacters (acctid, realmid, numchars) SELECT a.id, r.id, 0 FROM classicrealmd.account a JOIN classicrealmd.realmlist r WHERE a.username='$uEsc';" | Out-Null
...
"@)
    ...
    $d.FindName("BtnSave").Add_Click({
        ...
        Invoke-AuthQuery "UPDATE classicrealmd.account SET s='$($srp.SHex)',v='$($srp.VHex)',sessionkey=NULL WHERE id=$($acc.Id);" | Out-Null
        $d.DialogResult = $true
    })
    return ($d.ShowDialog() -eq $true)
}
...
'@
    ...
    $d.FindName("BtnSave").Add_Click({
        ...
        if ($newUser -ne $acc.Username) {
            ...
            $exists = Invoke-AuthQuery "SELECT COUNT(*) FROM classicrealmd.account WHERE username='$uEsc' AND id<>$($acc.Id);"
        ...
        Invoke-AuthQuery "UPDATE classicrealmd.account SET username='$uEsc', email='$eEsc', gmlevel=$newLvl WHERE id=$($acc.Id);" | Out-Null
        $d.DialogResult = $true
    })
    return ($d.ShowDialog() -eq $true)
}
...
$BtnAccDelete.Add_Click({
    ...
    $charRows  = Invoke-AuthQuery "SELECT guid FROM classiccharacters.characters WHERE account=$($acc.Id);"
    $charGuids = @($charRows | Where-Object { $_ -match '^\d+$' } | ForEach-Object { $_.Trim() })

    if ($charGuids.Count -gt 0) {
        $cg = $charGuids -join ','

        # Mail items and auction references (must precede item_instance deletion)
        Invoke-AuthQuery "
            DELETE FROM classiccharacters.mail_items WHERE mail_id IN (SELECT id FROM classiccharacters.mail WHERE receiver IN ($cg));
            DELETE FROM classiccharacters.item_loot WHERE owner_guid IN ($cg);
            DELETE FROM classiccharacters.auction WHERE itemguid IN (SELECT guid FROM classiccharacters.item_instance WHERE owner_guid IN ($cg));
        " | Out-Null

        # Item instances and mail
        Invoke-AuthQuery "
            DELETE FROM classiccharacters.item_instance WHERE owner_guid IN ($cg);
            DELETE FROM classiccharacters.mail WHERE receiver IN ($cg);
        " | Out-Null

        # Pet sub-tables (must precede character_pet)
        Invoke-AuthQuery "
            DELETE FROM classiccharacters.pet_aura           WHERE guid IN (SELECT id FROM classiccharacters.character_pet WHERE owner IN ($cg));
            DELETE FROM classiccharacters.pet_spell          WHERE guid IN (SELECT id FROM classiccharacters.character_pet WHERE owner IN ($cg));
            DELETE FROM classiccharacters.pet_spell_cooldown WHERE guid IN (SELECT id FROM classiccharacters.character_pet WHERE owner IN ($cg));
            DELETE FROM classiccharacters.character_pet WHERE owner IN ($cg);
        " | Out-Null

        # GM tickets and surveys
        Invoke-AuthQuery "
            DELETE FROM classiccharacters.gm_surveys WHERE ticketid IN (SELECT id FROM classiccharacters.gm_tickets WHERE author_guid IN ($cg));
            DELETE FROM classiccharacters.gm_tickets WHERE author_guid IN ($cg);
        " | Out-Null

        # Social, groups, guilds, petitions, PvP, corpse
        Invoke-AuthQuery "
            DELETE FROM classiccharacters.guild_member WHERE guid IN ($cg);
            DELETE FROM classiccharacters.group_member WHERE memberGuid IN ($cg);
            DELETE FROM classiccharacters.petition_sign WHERE playerguid IN ($cg) OR ownerguid IN ($cg);
            DELETE FROM classiccharacters.petition WHERE ownerguid IN ($cg);
            DELETE FROM classiccharacters.pvpstats_players WHERE character_guid IN ($cg);
            DELETE FROM classiccharacters.corpse WHERE player IN ($cg);
        " | Out-Null

        # All character_* guid-based tables
        Invoke-AuthQuery "
            DELETE FROM classiccharacters.character_account_data WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_action WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_aura WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_battleground_data WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_forgotten_skills WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_gifts WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_homebind WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_honor_cp WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_instance WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_inventory WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_queststatus WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_queststatus_weekly WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_reputation WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_skills WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_social WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_spell WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_spell_cooldown WHERE guid IN ($cg);
            DELETE FROM classiccharacters.character_stats WHERE guid IN ($cg);
        " | Out-Null

        # Main characters table
        Invoke-AuthQuery "DELETE FROM classiccharacters.characters WHERE account=$($acc.Id);" | Out-Null
    }

    # Account-level data in characters DB
    Invoke-AuthQuery "
        DELETE FROM classiccharacters.account_data WHERE account=$($acc.Id);
        DELETE FROM classiccharacters.account_instances_entered WHERE AccountId=$($acc.Id);
        DELETE FROM classiccharacters.character_tutorial WHERE account=$($acc.Id);
    " | Out-Null

    # Auth account-level data
    Invoke-AuthQuery "
        DELETE FROM classicrealmd.account_banned WHERE account_id=$($acc.Id);
        DELETE FROM classicrealmd.realmcharacters WHERE acctid=$($acc.Id);
        DELETE FROM classicrealmd.account WHERE id=$($acc.Id);
    " | Out-Null
...
$BtnShortcut.Add_Click({
    ...
    $ico = "$iconsDir\$($Script:theme.AppIcon)"; if (Test-Path $ico) { $sc.IconLocation = $ico }
    $sc.Save()
...
function Apply-ThemePreset([string]$expKey) {
    $dst = "$scriptsDir\Theme.txt"
    # Write only the ActiveTheme selector; colors are loaded from Themes\Exp*.txt
    @(
        "# Active theme for the Launcher. Edit here or use the Theme selector"
        "# in the Maintenance section. Restart the Launcher to apply changes."
        "#"
        "# Available themes:"
        "#   Exp1 - Classic (WoW Classic)"
        "#   Exp2 - The Burning Crusade"
        "#   Exp3 - Wrath of the Lich King"
        ""
        "ActiveTheme = $expKey"
    ) | Set-Content $dst -Encoding UTF8
    $Script:theme['ActiveTheme'] = $expKey
}

function Refresh-ThemeSelector {
    $active  = if ($Script:theme.ContainsKey('ActiveTheme')) { $Script:theme.ActiveTheme } else { 'Exp1' }
    $brush1  = [Windows.Media.SolidColorBrush]([Windows.Media.Color]::FromRgb(0xFF, 0xD1, 0x00))  # Classic gold
    $brush2  = [Windows.Media.SolidColorBrush]([Windows.Media.Color]::FromRgb(0x56, 0xE0, 0x5F))  # TBC green
    $brush3  = [Windows.Media.SolidColorBrush]([Windows.Media.Color]::FromRgb(0x58, 0xB4, 0xF0))  # WotLK blue
    $neutral = [Windows.Media.SolidColorBrush]([Windows.Media.Color]::FromRgb(0x4E, 0x50, 0x58))  # inactive border

    $ThemeActive1.Visibility  = if ($active -eq 'Exp1') { 'Visible' } else { 'Collapsed' }
    $ThemeActive2.Visibility  = if ($active -eq 'Exp2') { 'Visible' } else { 'Collapsed' }
    $ThemeActive3.Visibility  = if ($active -eq 'Exp3') { 'Visible' } else { 'Collapsed' }
    $ThemeCard1.BorderBrush   = if ($active -eq 'Exp1') { $brush1 } else { $neutral }
    $ThemeCard2.BorderBrush   = if ($active -eq 'Exp2') { $brush2 } else { $neutral }
    $ThemeCard3.BorderBrush   = if ($active -eq 'Exp3') { $brush3 } else { $neutral }
}

$BtnTheme1.Add_Click({
    if (Show-Confirm "Apply the Classic theme?`nThe launcher must be restarted for the changes to take effect.") {
        Apply-ThemePreset 'Exp1'
        Refresh-ThemeSelector
    }
})
$BtnTheme2.Add_Click({
    if (Show-Confirm "Apply the Burning Crusade theme?`nThe launcher must be restarted for the changes to take effect.") {
        Apply-ThemePreset 'Exp2'
        Refresh-ThemeSelector
    }
})
$BtnTheme3.Add_Click({
    if (Show-Confirm "Apply the Wrath of the Lich King theme?`nThe launcher must be restarted for the changes to take effect.") {
        Apply-ThemePreset 'Exp3'
        Refresh-ThemeSelector
    }
})
...
$Script:embedTimer.Add_Tick({
    ...
    $allDone = $true
    foreach ($pName in @("realmd","mangosd")) {
...
# resetting the attribute (mangosd takes several minutes to load).
...
$win.Add_Closing({
    ...
    $anyRunning = (Test-Proc "mysqld") -or (Test-Proc "realmd") -or (Test-Proc "mangosd") -or (Test-Proc "httpd")
    if ($anyRunning) {
