...
function Read-Directories {
    $defaults = @{
        ...
        Website  = "$mainfolder\Tools\Apache24\Apache_Server.bat"
        HeidiSQL = "$mainfolder\Tools\HeidiSQL\heidisql.exe"
        Notepad  = "$mainfolder\Tools\Notepad++\notepad++.exe"
        Updater      = "$mainfolder\Update.bat"
...
function Start-BnetServer {
    ...
    if (Test-Path $exe) { Start-Process $exe -WorkingDirectory (Split-Path $exe) -WindowStyle Minimized; Start-EmbedTimer }
}
...
function Start-WebServer {
    if (Test-Proc "spp-httpd") { return }
    $bat = $Script:srvDirs.Website
    if (Test-Path $bat) { Start-Process cmd "/c `"$bat`"" -WorkingDirectory (Split-Path $bat) -WindowStyle Hidden }
}
function Stop-WebServer { Stop-Process -Name "spp-httpd" -Force -ErrorAction SilentlyContinue }
...
<Window
  ...
  <Grid>
    ...
    <Border Grid.Column="0" Background="#0A1210">
      <Grid>
        ...
        <Border Grid.Row="0" Padding="18,22,18,18" BorderBrush="#1A3020" BorderThickness="0,0,0,1">
          <StackPanel>
            <TextBlock Name="TxtAppName" FontSize="19" FontWeight="Bold" Foreground="#00E676"/>
        ...
        <StackPanel Grid.Row="1" Margin="0,10,0,0">
          ...
          <Button Name="NavAccounts" Content="  Create Accounts" Style="{StaticResource NavBtn}"/>
    ...
    <Grid Grid.Column="1">
            ...
            <Grid>
              ...
              <StackPanel Grid.Column="0" Orientation="Horizontal" VerticalAlignment="Center">
                ...
                <StackPanel>
                  ...
                  <TextBlock Name="SrvLblWeb" Text="spp-httpd.exe - Stopped" Foreground="#4A7A5A" FontSize="11"/>
                </StackPanel>
              </StackPanel>
              <StackPanel Grid.Column="1" Orientation="Horizontal">
        ...
        <StackPanel Margin="24,22,24,24">
            ...
            <StackPanel>
              ...
              <Grid>
                ...
                <StackPanel Grid.Column="0">
                  ...
                  <TextBlock Name="RealmMsg" Text="" Foreground="#00E676" FontSize="11" Margin="0,6,0,0"/>
                </StackPanel>
                <!-- External Address -->
                <StackPanel Grid.Column="2">
                  ...
                  <TextBlock Name="AddressMsg" Text="" Foreground="#00E676" FontSize="11" Margin="0,6,0,0"/>
                </StackPanel>
              </Grid>
            </StackPanel>
          </Border>
          ...
          <Border Style="{StaticResource Card}">
            <StackPanel>
              ...
              <WrapPanel>
                <Button Name="BtnMgmtTool"  Content="Management Tool"    Style="{StaticResource BtnGray}" Margin="0,0,8,6"/>
      ...
      <!-- CREATE ACCOUNTS -->
      <ScrollViewer Name="PageAccounts" VerticalScrollBarVisibility="Auto" Visibility="Collapsed">
        <StackPanel Margin="24,22,24,24">

          <TextBlock Text="Create Accounts" FontSize="20" FontWeight="Bold" Foreground="#EDFFF2"/>
          <TextBlock Text="Choose a method to create player accounts" FontSize="12"
                     Foreground="#4A7A5A" Margin="0,3,0,18"/>

          <Border Style="{StaticResource Card}" Margin="0,0,0,12">
            <StackPanel>
              <TextBlock Text="Option 1 :  Management Tool" Foreground="#EDFFF2" FontSize="13" FontWeight="SemiBold" Margin="0,0,0,4"/>
              <TextBlock Text="Use the SPP Management desktop application to create and manage accounts locally."
                         Foreground="#4A7A5A" FontSize="11" Margin="0,0,0,14"/>
              <Button Name="BtnAccMgmt" Content="Open Management Tool"
                      Style="{StaticResource BtnGreen}" HorizontalAlignment="Left" MinWidth="180"/>
            </StackPanel>
          </Border>

          <Border Style="{StaticResource Card}">
            <StackPanel>
              <TextBlock Text="Option 2 :  Web Registration" Foreground="#EDFFF2" FontSize="13" FontWeight="SemiBold" Margin="0,0,0,4"/>
              <TextBlock Text="Open the local website to register accounts through the web interface."
                         Foreground="#4A7A5A" FontSize="11" Margin="0,0,0,14"/>
              <Button Name="BtnAccWebsite" Content="Open Website"
                      Style="{StaticResource BtnGray}" HorizontalAlignment="Left" MinWidth="180"/>
            </StackPanel>
          </Border>
      ...
      <ScrollViewer Name="PageService" VerticalScrollBarVisibility="Auto" Visibility="Collapsed">
        <StackPanel Margin="24,22,24,24">
          ...
          <Border Style="{StaticResource Card}">
            <StackPanel>
              <TextBlock Text="Update Server"
                         Foreground="#EDFFF2" FontSize="13" FontWeight="SemiBold"/>
              <TextBlock Text="Runs Update.bat in the server's root folder."
                         Foreground="#4A7A5A" FontSize="11" Margin="0,4,0,12"/>
              <Button Name="BtnUpdate" Content="Run Updater"
                      Style="{StaticResource BtnRed}" HorizontalAlignment="Left" MinWidth="200"/>
            </StackPanel>
          </Border>
...
catch { Write-Host "XAML Error: $_" -ForegroundColor Red; Read-Host; exit 1 }
...
function Refresh-Status {
    ...
    $SrvLblWeb.Text   = "spp-httpd.exe - "  + $SrvLblWeb.Text
...
$allPages   = @($PageMain, $PageServers, $PageSaves, $PageSettings, $PageAccounts, $PageService)
$allNavBtns = @($NavMain,  $NavServers,  $NavSaves,  $NavSettings,  $NavAccounts,  $NavService)
...
function Switch-Page([int]$idx) {
    ...
    if ($idx -eq 0) { Sync-Consoles }
    if ($idx -eq 2) { Refresh-Saves }
    if ($idx -eq 3) { $RealmInput.Text = Get-RealmName; $AddressInput.Text = Get-RealmAddress }
}

$NavMain.Add_Click(          { Switch-Page 0 })
$NavServers.Add_Click(       { Switch-Page 1 })
$NavSaves.Add_Click(         { Switch-Page 2 })
$NavSettings.Add_Click(      { Switch-Page 3 })
$NavAccounts.Add_Click(      { Switch-Page 4 })
...
$BtnMgmtTool.Add_Click({
    $exe = "$mainfolder\Tools\SPP LegionV2 Management\SPP-LegionV2-Management.exe"
    if (Test-Path $exe) { Start-Process $exe -WorkingDirectory (Split-Path $exe) }
...
$BtnAccMgmt.Add_Click({
    $exe = "$mainfolder\Tools\SPP LegionV2 Management\SPP-LegionV2-Management.exe"
    if (Test-Path $exe) { Start-Process $exe -WorkingDirectory (Split-Path $exe) }
    else { Show-Alert "Not found:`n$exe" }
})
$BtnAccWebsite.Add_Click({ Start-Process "http://127.0.0.1" })
    ...
    <StackPanel>
      ...
      <Border Background="#152218" CornerRadius="6" BorderThickness="1" BorderBrush="#2E5E3E" Padding="10,7" Margin="0,0,0,20">
        <TextBox Name="TxtInput" Background="Transparent" Foreground="#EDFFF2" BorderThickness="0"
                 FontSize="13" CaretBrush="#00E676" SelectionBrush="#00C853"/>
      </Border>
      <StackPanel Orientation="Horizontal" HorizontalAlignment="Right">
...
$BtnUpdate.Add_Click({
    $bat = $Script:srvDirs.Updater
    if (Test-Path $bat) {
        if (Show-Confirm "Run the updater?`nClose the updater window when it finishes.") {
            Start-Process cmd "/c `"$bat`"" -WorkingDirectory $mainfolder -Wait
        }
    } else { Show-Alert "Not found:`n$bat" }
})
...
$win.Add_Loaded({
    ...
    # Force taskbar icon via WM_SETICON — store at script scope to prevent GC invalidating the HICON
    if (Test-Path $iconPath) {
        try {
            $Script:icoSmall = [System.Drawing.Icon]::new($iconPath, 16, 16)
            $Script:icoBig   = [System.Drawing.Icon]::new($iconPath, 32, 32)
            [SPP.WinAPI]::SendMessage($Script:wpfHwnd, [uint32]0x0080, [IntPtr]0, $Script:icoSmall.Handle) | Out-Null  # ICON_SMALL
            [SPP.WinAPI]::SendMessage($Script:wpfHwnd, [uint32]0x0080, [IntPtr]1, $Script:icoBig.Handle)   | Out-Null  # ICON_BIG
        } catch {}
    }
    # Load name, version and app name from Scripts folder
    ...
    Refresh-Status
    $RealmInput.Text   = Get-RealmName
    $AddressInput.Text = Get-RealmAddress
    Refresh-Saves
})

# ── Console repositioning via LayoutUpdated ───────────────────────────────────
# LayoutUpdated fires after every completed WPF layout pass.
# ActualWidth/Height are always final here. Only calls MoveWindow (very cheap).
...
$win.Add_Closing({
    ...
    $anyRunning = (Test-Proc "mysqld") -or (Test-Proc "bnetserver") -or (Test-Proc "worldserver") -or (Test-Proc "spp-httpd")
    if ($anyRunning) {
